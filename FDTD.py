...
class fdtd_3d_systolic_block:
    ...
    def set_block(self, dt, dx, dy, dz, ep, mu, sigma_x=0, sigma_y=0, sigma_z=0):
        ...
        self.kez1 = (ep - 0.5 * sigma_z * dt) / (ep + 0.5 * sigma_z * dt)
        self.kez2 = dt / dx / (ep + 0.5 * sigma_z * dt)
        self.kez3 = dt / dy / (ep + 0.5 * sigma_z * dt)
        ...
        self.khz1 = (mu - 0.5 * sigma_z * mu / ep * dt) / (mu + 0.5 * sigma_z * mu / ep * dt)
        self.khz2 = dt / dy / (mu + 0.5 * sigma_z * mu / ep * dt)
        self.khz3 = dt / dx / (mu + 0.5 * sigma_z * mu / ep * dt)
    ...
    def update_parameters(self):
        ...
        self.kez1 = (self.ep - 0.5 * self.sigma_z * self.dt) / (self.ep + 0.5 * self.sigma_z * self.dt)
        self.kez2 = self.dt / self.dx / (self.ep + 0.5 * self.sigma_z * self.dt)
        self.kez3 = self.dt / self.dy / (self.ep + 0.5 * self.sigma_z * self.dt)
        ...
        self.khz1 = (self.mu - 0.5 * self.sigma_z * self.mu / self.ep * self.dt) / \
                    (self.mu + 0.5 * self.sigma_z * self.mu / self.ep * self.dt)
        self.khz2 = self.dt / self.dy / (self.mu + 0.5 * self.sigma_z * self.mu / self.ep * self.dt)
        self.khz3 = self.dt / self.dx / (self.mu + 0.5 * self.sigma_z * self.mu / self.ep * self.dt)
